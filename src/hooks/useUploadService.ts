...
const useUploadService = () => {
  ...
  const getAwsCredentials = useCallback(
    async (folderPrefix: string): Promise<IAwsCredentials | null> => {
      try {
        hasShownErrorToastRef.current = false;

        console.log(
          "Fetching AWS credentials with folderPrefix:",
          folderPrefix
        );
  ...
  const uploadToS3 = useCallback(
    async (
      ...
      try {
        ...
        const region = credentials?.S3Region || credentials?.region;
        const bucket = credentials?.S3BucketName;
        ...
        throw new Error(
          "Upload failed. Please check your credentials or network."
        );
      }
    },
    []
